...
export default defineEventHandler(async (event) => {
  ...
  const file = formData.get('keysafeImage') as File
  ...
  // Increase file size limit to 20MB (will be compressed to WEBP if Sharp works)
  ...
  try {
    ...
    // Try to use Sharp for image processing, fallback to original if Sharp is not available or fails
    if (sharp) {
      try {
    ...
    const filePath = `keysafe/${fileName}`
    ...
    if (uploadError) {
      ...
      throw createError({
        statusCode: 500,
        statusMessage: `Failed to upload keysafe image: ${uploadError.message || JSON.stringify(uploadError)}`
      })
    }
    ...
    console.error('Keysafe image upload error:', error)
    ...
    throw createError({
      statusCode: 500,
      statusMessage: `Failed to upload keysafe image: ${errorMessage}`
    })
  }
})
